...
function bindFace(){
	...
	var $faceBox=$("<div></div>");
	$faceBox.addClass("faceBox");
	$faceBox.appendTo("#face");
	$.each(userFaces, function(i,e) {
		var $img=$("<img src='faces/"+i+"' alt="+e+" />");
		$img.appendTo($faceBox);
...
function bindFriends(){
	...
	$ul.addClass("ulStyle");
	$ul.appendTo("#friends");
	$("#friends").on("mouseenter",function(){
...
function bindNum(){
	...
	function checkNum(){
		var str=$("#msgTxt").val();
		if(str.length<=140){
			$("#txtNum").html("还能输入<em>140</em>字");
			$("#txtNum em").text(140-str.length);
		}else{
			$("#txtNum").html("已超出<em></em>字");
			$("#txtNum em").text(str.length-140);
		}
	}
}
